...
import { loadProtos, createRegistrar, type RegisterFn } from '../src/helpers'
import {
  isGrpcHealthPackage,
...
describe('helpers:loadProtos', () => {
  const healthProto = path.join(process.cwd(), 'src/protos/grpc/health/v1/health.proto')
  ...
  it('returns unknown without assert (type negative)', () => {
    ...
    const _typed: GrpcHealthPackage = loadProtos({ files: [healthProto] })
  })
})
...
describe('helpers:createRegistrar', () => {
  ...
  it('enforces RegisterFn signature (type negative)', () => {
    ...
    const _badRegistrar = createRegistrar((x: number) => {
      void x
    })
  })
})
